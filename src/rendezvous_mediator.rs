...
///
/// Waiting also costs nothing, because the two gateways that lose us the mapping need opposite
/// things and a long timeout serves both. One that answers with RST kills the attempt at once,
/// leaving nothing to retransmit, and only a fresh attempt can punch again - which the loop makes.
/// One that drops the SYN in silence leaves the socket in SYN_SENT, where it holds the mapping
/// open and the kernel re-sends the SYN on its own; a shorter timeout would only take that away
/// and buy attempts that are not needed.
